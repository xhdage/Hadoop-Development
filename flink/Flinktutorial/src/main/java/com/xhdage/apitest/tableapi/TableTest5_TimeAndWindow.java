...
//        });
        ...
        // 4、将流转换为表，定义时间特性,processtime
//        Table table = tableEnv.fromDataStream(dataStream, "id, timestamp as ts, temperature as temp, pt.proctime ");
//         Table table = tableEnv.fromDataStream(dataStream, "id, timestamp.rowtime, temperature as temp");
        ...
        // 5.2 开窗函数，Over Window
        // table api
        Table overResult = table.window(Over.partitionBy("id").orderBy("rt").preceding("2.rows").as("ow"))
...
//        tableEnv.toRetractStream(resultSqlTable, Row.class).print("resultSqlTable");
        tableEnv.toAppendStream(overResult, Row.class).print("resultTable");
        tableEnv.toRetractStream(overSqlResult, Row.class).print("resultSqlTable");
